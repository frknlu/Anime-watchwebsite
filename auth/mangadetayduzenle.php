...
<form  name="formara" method="post" action="/auth/mangadetayduzenle.php?adim=ara">
...
## Sayfa Degiskeninin alalim.
	...
	$bul		=	mysqli_query($con,"select * from manga order by id DESC limit $nereden,$kacar");
		...
		<td>".$durumdeger."</td>
		<td><a href='mangabolumlistele.php?id=".$cek["essizid"]."'><font color='blue'>Düzenle</font></a>
	  </td>
		</tr>
		";
		}
	...
	for ($i=1; $i<=$ssayisi; $i++){
		echo "<span style='text-align:center;margin:5px; border:1px dotted orange'><a href='mangadetayduzenle.php?s={$i}'";
		if($i == $sayfa){
			echo "class='aktif'";
		}
		echo ">{$i}</a></span>";
...
$bul=mysqli_num_rows($deger);
  ...
  if(isset($bul)) {
    while($yaz=mysqli_fetch_array($deger)) {
	  echo  "
	  <span><a href='mangabolumlistele.php?id=".$yaz["essizid"]."'>Düzenle -> ".$yaz["ad"]." | Tarih:".$yaz["guncellemetarihi"]." | Durum:".$yaz["durum"]." </a><hr></span>";
